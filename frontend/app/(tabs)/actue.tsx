...
import { ArrowUpRight, Bookmark, Radar, ShieldCheck, Sparkles, Zap } from "lucide-react-native";
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {loading ? (
          ...
          <>
            ...
            {highlights?.radar?.length ? <RadarStrip radar={highlights.radar} /> : null}
...
function RadarStrip({ radar }: { radar: NonNullable<EditorialHighlights["radar"]> }) {
  const router = useRouter();
  const { colors } = useTheme();
  return (
    <Animated.View entering={FadeInDown.delay(140).duration(420)} style={[styles.radarBox, { backgroundColor: colors.surface, borderColor: colors.borderSubtle }] }>
      <View style={styles.radarHead}>
        <Radar size={17} color={colors.coral} strokeWidth={2.5} />
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.radarRow}>
        {radar.slice(0, 6).map((r, i) => {
          const targetId = typeof r.id === "string" && r.id.length > 0 ? r.id : null;
          return (
          <TouchableOpacity key={`${r.id ?? r.title}-${i}`} onPress={() => targetId ? router.push({ pathname: "/news/[id]", params: { id: targetId } }) : undefined} style={[styles.radarCard, { backgroundColor: colors.bg, borderColor: colors.borderSubtle }]} activeOpacity={0.82} disabled={!targetId}>
            <Text style={[styles.radarStatus, { color: colors.coral }]}>{r.label ?? "À surveiller"}</Text>
            <Text numberOfLines={2} style={[styles.radarCardTitle, { color: colors.textPrimary }]}>{r.title}</Text>
            <Text numberOfLines={2} style={[styles.radarAction, { color: colors.textSecondary }]}>{r.action}</Text>
            <Text style={[styles.radarOpen, { color: colors.coral }]}>{targetId ? "Ouvrir l’article" : "Info à surveiller"}</Text>
          </TouchableOpacity>
          );
        })}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  radarHead: { flexDirection: "row", alignItems: "center", gap: 7, marginBottom: spacing.sm },
  radarTitle: { fontFamily: fonts.serif, fontSize: 20, lineHeight: 24 },
  radarRow: { gap: 8, paddingRight: spacing.md },
  radarCard: { width: 190, borderWidth: 1, borderRadius: radius.lg, padding: spacing.md, minHeight: 118 },
  radarStatus: { fontFamily: fonts.bodyBold, fontSize: 10, letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 6 },
  radarCardTitle: { fontFamily: fonts.serif, fontSize: 16, lineHeight: 20 },
  radarAction: { fontFamily: fonts.body, fontSize: 11, lineHeight: 16, marginTop: 6 },
